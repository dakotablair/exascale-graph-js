...
class Collection {
   ...
   * @param {*} graphObject - a cytoscape graph object, which we use to create a collection
   * @param {*} dataObject
   * @memberof Collection
   */
  ...
  collectSelectedTableNodes(dt) {
    ...
    dt.rows({ selected: true })
      ...
      .each((d) => {
        ...
        const node = this._graph.collectNode(id);
        if (node) {
  ...
  collectSelectedTableEdges(dt) {
    dt.rows({ selected: true })
      ...
      .each((d) => {
        ...
        const id_arr = [d.source, d.target];
        id_arr.forEach((id) => {
          this._data.collection[id] = this._data.node[id];
          const node = this._graph.collectNode(id);
          if (node) {
  ...
  discardSelectedTableNodes(dt) {
    ...
    dt.rows({ selected: true })
      ...
      .each((d) => {
        ...
        const node = this._graph.discardNode(id);
        if (node) {
