...
// register all the datatables stuff
dt(window, $);
buttons(window, $);
buttons_bs(window, $);
...
function refreshTable(tableID) {
  Object.keys(window.kbase.tableIx).forEach((k) => {
    if (!tableID || k === tableID) {
      const thisTable = window.kbase.tableIx[k].DataTable();
 ...
 */
function addButtonListener(schema, tableID) {
  ...
  $(`#${tableID}_table tbody`)
    ...
    .on('click', '.view_button', (e) => {
      ...
      if (row.child.isShown()) {
        ...
        // Open this row
        const str = formatData(schema, row.data());
        row.child(str).show();
        tr.addClass('shown');
        $(e.target).text('Hide');
      }
 ...
 *
 * @param {object}   datasetConfig   schema for all data (nodes and edges)
 * @returns {object} tableIx, in the form { tableKey: $jQueryTableDOMElement }
 */
function initTables(datasetConfig) {
  const tableIDs = ['edge', 'node', 'collection'];
  const tableIx = {};

  tableIDs.forEach((tableID) => {
    const schema =
      tableID === 'node' || tableID === 'collection'
        ? datasetConfig['djornl_node']
        : datasetConfig['djornl_edge'];
    const config = tableConfig(schema, tableID);
    config.data = [];
    // initialise the table
    $(`#${tableID}_table`).DataTable(config);
    addButtonListener(schema, tableID);
